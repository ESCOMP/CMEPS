  ...
  subroutine state_setexport(exportState, rc)
    ...
    do nf = 2,fldsFrAtm_num
       if (fldsFrAtm(nf)%ungridded_ubound == 0) then
          call field_setexport(exportState, trim(fldsFrAtm(nf)%stdname), lon, lat, nf=field_num, rc=rc)
          if (chkerr(rc,__LINE__,u_FILE_u)) return
          field_num = field_num + 1
       else
          do nind = 1,fldsFrAtm(nf)%ungridded_ubound
             call field_setexport(exportState, trim(fldsFrAtm(nf)%stdname), lon, lat, nf=field_num, &
