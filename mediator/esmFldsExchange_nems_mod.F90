    ...
    do n = 1,size(flds)
       ...
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compocn)        , trim(fldname), rc=rc) .and. &
             ...
             call addmapFrom(compwav, trim(fldname), compocn, mapbilnr_nstod, 'one', 'unset')
    ...
    do n = 1,size(flds)
     ...
     do n = 1,size(flds)
        ...
        if (phase == 'advertise') then
           ...
           if ( fldchk(is_local%wrap%FBexp(compwav)        , trim(fldname), rc=rc) .and. &
                fldchk(is_local%wrap%FBImp(compice,compice), trim(fldname), rc=rc)) then
              call addmapFrom(compice, trim(fldname), compwav, mapbilnr_nstod , 'one', 'unset')
              call addmrgTo(compwav, trim(fldname), mrg_from=compice, mrg_fld=trim(fldname), mrg_type='copy')
           end if
        end if
      end do
      deallocate(flds)
     ...
     do n = 1,size(flds)
        ...
        if (phase == 'advertise') then
           ...
           if ( fldchk(is_local%wrap%FBexp(compwav)        , trim(fldname), rc=rc) .and. &
                fldchk(is_local%wrap%FBImp(compocn,compocn), trim(fldname), rc=rc)) then
              call addMapFrom(compocn, trim(fldname), compwav, mapbilnr_nstod , 'one', 'unset')
