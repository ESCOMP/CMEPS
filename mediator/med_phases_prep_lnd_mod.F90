module med_phases_prep_lnd_mod
  ...
  use esmFlds               , only : max_icesheets, num_icesheets, compglc1, compglc2
...
!================================================================================================
    ...
    if (ncnt > 0) then
       ...
       do n1 = 1,ncomps
          ...
          if (is_local%wrap%med_coupling_active(n1,complnd)) then
             if (n1 /= compglc1 .and. n1 /= compglc2) then  ! TODO: improve this logic
       ...
       do ns = 1,num_icesheets
          if (is_local%wrap%med_coupling_active(compglc(ns),complnd)) then
             if (first_call) then
                ...
                call map_glc2lnd_init(gcomp, ns, rc=rc)
                if (ChkErr(rc,__LINE__,u_FILE_u)) return
                call map_glc2lnd(gcomp, ns, rc=rc)
                if (ChkErr(rc,__LINE__,u_FILE_u)) return
       ...
       end do ! end of loop over ice sheets (ns)
  ...
  subroutine map_glc2lnd( gcomp, rc)
    ...
    real(r8), pointer     :: frac_l_ec_sum(:,:) => null()       ! glc fractions on the land grid
    ...
    real(r8), pointer     :: topo_l_ec_sum(:,:) => null()       ! topo height in each land gridcell for each elev class
    ...
    call ESMF_FieldBundleGet(is_local%wrap%FBExp(complnd), fieldname=trim(Sg_icemask)
         field=lfield, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldGet(lfield, farrayptr=sg_icemask_sum, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    sg_icemask_sum(:) = 0._r8

    call ESMF_FieldBundleGet(is_local%wrap%FBExp(complnd), fieldname=trim(Sg_icemask_coupled_fluxes), &
         field=lfield, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    call ESMF_FieldGet(lfield, farrayptr=sg_icemask_coupled_fluxes_sum, rc=rc)
    if (chkerr(rc,__LINE__,u_FILE_u)) return
    sg_icemask_coupled_fluxes_sum(:) = 0._r8
    ...
    do ns = 1,max_icesheets
       ...
       call ESMF_FieldBundleGet(is_local%wrap%FBExp(compglc(ns),compglc(ns)), &
       ...
       if (chkerr(rc,__LINE__,u_FILE_u)) return
       sg_icemask_sum(:) = sg_icemask_sum(:) + icemask_l(:)

       ! Map Sg_icemask_coupled_fluxes (no elevation classes)
       call ESMF_FieldBundleGet(is_local%wrap%FBExp(compglc(ns),compglc(ns)), &
            fieldname=trim(Sg_icemask_coupled_fluxes), field=fieldsrc, rc=rc)
       if (chkerr(rc,__LINE__,u_FILE_u)) return
       call med_map_field( &
            field_src=field_src, &
            field_dst=field_dst, &
