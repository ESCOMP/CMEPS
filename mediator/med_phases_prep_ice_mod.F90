...
!-----------------------------------------------------------------------------
  ...
  subroutine med_phases_prep_ice(gcomp, rc)
    ...
    if (ncnt > 0) then
       ...
       ! This is a prep phase for the next two sections, Sa_dens and Sa_ptem custom calculations
       ! Interpolate Sa_pbot and Sa_tbot if needed
       if ((.not. fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_ptem',rc=rc)   .and. &
                  fldchk(is_local%wrap%FBExp(compice)        , 'Sa_ptem',rc=rc))  .or.  &
           (.not. fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_dens',rc=rc)   .and. &
                  fldchk(is_local%wrap%FBExp(compice)        , 'Sa_dens',rc=rc))) then
          ...
          if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_pbot',rc=rc) .and. &
             ...
             ! interpolate fields not done as part of prep_ice
             allocate(fldnames(2))
             fldnames = (/'Sa_pbot','Sa_tbot'/)
             do n = 1,size(fldnames)
                if (.not. fldchk(is_local%wrap%FBExp(compice), trim(fldnames(n)), rc=rc)) then
                   call ESMF_LogWrite(trim(subname)//": mapping "//trim(fldnames(n))//" for custom calculation", ESMF_LOGMSG_INFO)
                   call med_fldList_GetFldInfo(fldListFr(compatm),trim(fldnames(n)),fldnum)
                   mapindex = fldListFr(compatm)%flds(fldnum)%mapindex(compice)
                   if (.not. med_map_RH_is_created(is_local%wrap%RH(compatm,compice,:),mapindex,rc)) then
                      call ESMF_LogWrite(trim(subname)//": ERROR "//trim(mapnames(mapindex))//" RH not available for atm->ice "//trim(fldnames(n)), &
                           ESMF_LOGMSG_ERROR, line=__LINE__, file=u_FILE_u)
                      rc = ESMF_FAILURE
                     return
                   end if
                   call med_map_FB_Field_Regrid( &
                        is_local%wrap%FBImp(compatm,compatm), trim(fldnames(n)), &
                        is_local%wrap%FBImp(compatm,compice), trim(fldnames(n)), &
                        is_local%wrap%RH(compatm,compice,:), mapindex, rc=rc)
                   if (chkerr(rc,__LINE__,u_FILE_u)) return
                endif
             enddo
             deallocate(fldnames)
             ...
             call ESMF_LogWrite(trim(subname)//": custom calculations cannot be computed due to missing fields", &
                                ESMF_LOGMSG_ERROR, line=__LINE__, file=u_FILE_u)
             rc = ESMF_FAILURE
             return
          endif
       ...
       ! if Sa_dens is not sent by the atm but is required by the ice
       ! compute air density as a custom calculation
       ! requires Sa_shum, Sa_pbot, Sa_tbot from atm, will abort if it can't be calculated
       if (.not. fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_dens',rc=rc) .and. &
                 fldchk(is_local%wrap%FBExp(compice)        , 'Sa_dens',rc=rc)) then

          if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_shum',rc=rc) .and. &
              fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_pbot',rc=rc) .and. &
              fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_tbot',rc=rc)) then

             call ESMF_LogWrite(trim(subname)//": computing air density as a custom calculation", ESMF_LOGMSG_INFO)

             ! interpolate fields if not done as part of prep_ice
             ! Sa_pbot, Sa_tbot done above, do Sa_shum here
             allocate(fldnames(1))
             fldnames = (/'Sa_shum'/)
             do n = 1,size(fldnames)
                if (.not. fldchk(is_local%wrap%FBExp(compice), trim(fldnames(n)), rc=rc)) then
             ...
             call ESMF_LogWrite(trim(subname)//": air density to ice cannot be computed", &
                                ESMF_LOGMSG_ERROR, line=__LINE__, file=u_FILE_u)
             rc = ESMF_FAILURE
             return
          end if
       ...
       ! requires Sa_pbot, Sa_tbot from atm, will abort if it can't be calculated
       if (.not. fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_ptem',rc=rc) .and. &
          ...
          if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_pbot',rc=rc) .and. &
             ...
             ! interpolate fields if not done as part of prep_ice
             ! Sa_pbot and Sa_tbot done above so we should be all set
             ...
             call ESMF_LogWrite(trim(subname)//": potential temperature to ice cannot be computed", &
                                ESMF_LOGMSG_ERROR, line=__LINE__, file=u_FILE_u)
             rc = ESMF_FAILURE
             return
          endif
