...
!================================================================================
  ...
  subroutine esmFldsExchange_cesm(gcomp, phase, rc)
    ...
    if (phase == 'advertise') then
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       do n = 1,ncomps
          call addfldFrom(n, trim(cvalue))
          call addfldTo(n, trim(cvalue))
       end do
    end if
    ...
    !----------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(complnd, 'Sl_lfrin')
       call addfldFrom(compocn, 'So_omask')
       call addfldFrom(compice, 'Si_imask')
       do ns = 1,is_local%wrap%num_icesheets
          call addfldFrom(compglc(ns), 'Sg_area')
       end do
    else
       call addmapFrom(compocn, 'So_omask', compice,  mapfcopy, 'unset', 'unset')
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_u')
       call addFldFrom(compatm, 'Sa_v')
       call addFldFrom(compatm, 'Sa_z')
       call addFldFrom(compatm, 'Sa_tbot')
       call addFldFrom(compatm, 'Sa_pbot')
       call addFldFrom(compatm, 'Sa_shum')
       call addFldFrom(compatm, 'Sa_ptem')
       call addFldFrom(compatm, 'Sa_dens')
       if (flds_wiso) then
          call addFldFrom(compatm, 'Sa_shum_wiso')
       end if
    else
       if (is_local%wrap%aoflux_grid == 'ogrid') then
          if (mapuv_with_cart3d) then
             call addmapFrom(compatm, 'Sa_u' , compocn, mappatch_uv3d, 'one', atm2ocn_map)
             call addMapFrom(compatm, 'Sa_v' , compocn, mappatch_uv3d, 'one', atm2ocn_map)
          else
             call addMapFrom(compatm, 'Sa_u' , compocn, mappatch, 'one', atm2ocn_map)
             call addMapFrom(compatm, 'Sa_v' , compocn, mappatch, 'one', atm2ocn_map)
          end if
          call addMapFrom(compatm, 'Sa_z'   , compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_tbot', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_pbot', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_shum', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_ptem', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_dens', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_pslv', compocn, mapbilnr, 'one', atm2ocn_map)
          if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Sa_shum_wiso', rc=rc)) then
             call addMapFrom(compatm, 'Sa_shum_wiso', compocn, mapbilnr, 'one', atm2ocn_map)
          end if
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Fall_swnet')
       call addfldFrom(compice, 'Faii_swnet')
       call addFldFrom(compatm, 'Faxa_swnet')
    else
       if (fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swnet', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swnet', compice, mapconsf, 'one'  , atm2ice_map)
          call addMapFrom(compatm, 'Faxa_swnet', compocn, mapconsf, 'one'  , atm2ocn_map)
       end if
       if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_swnet', rc=rc)) then
          call addMapFrom(compice, 'Faii_swnet', compocn, mapfcopy, 'unset', 'unset')
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_z')
       call addfldTo(complnd, 'Sa_z')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_z', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_z', rc=rc)) then
          call addMapFrom(compatm, 'Sa_z', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_z', mrg_from=compatm, mrg_fld='Sa_z', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_topo')
       call addfldTo(complnd, 'Sa_topo')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_topo', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_topo', rc=rc)) then
          call addMapFrom(compatm, 'Sa_topo', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_topo', mrg_from=compatm, mrg_fld='Sa_topo', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_u')
       call addfldTo(complnd, 'Sa_u')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_u', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_u', rc=rc)) then
          call addMapFrom(compatm, 'Sa_u', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_u', mrg_from=compatm, mrg_fld='Sa_u', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_v')
       call addfldTo(complnd, 'Sa_v')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_v', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_v', rc=rc)) then
          call addMapFrom(compatm, 'Sa_v', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_v', mrg_from=compatm, mrg_fld='Sa_v', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_pbot')
       call addfldTo(complnd, 'Sa_pbot')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_pbot', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_pbot', rc=rc)) then
          call addMapFrom(compatm, 'Sa_pbot', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_pbot', mrg_from=compatm, mrg_fld='Sa_pbot', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_o3')
       call addfldTo(complnd, 'Sa_o3')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_o3', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_o3', rc=rc)) then
          call addMapFrom(compatm, 'Sa_o3', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_o3', mrg_from=compatm, mrg_fld='Sa_o3', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_tbot')
       call addfldTo(complnd, 'Sa_tbot')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_tbot', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_tbot', rc=rc)) then
          call addMapFrom(compatm, 'Sa_tbot', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_tbot', mrg_from=compatm, mrg_fld='Sa_tbot', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_ptem')
       call addfldTo(complnd, 'Sa_ptem')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_ptem', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_ptem', rc=rc)) then
          call addMapFrom(compatm, 'Sa_ptem', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_ptem', mrg_from=compatm, mrg_fld='Sa_ptem', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_shum')
       call addfldTo(complnd, 'Sa_shum')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_shum', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_shum', rc=rc)) then
          call addMapFrom(compatm, 'Sa_shum', complnd, mapbilnr, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Sa_shum', mrg_from=compatm, mrg_fld='Sa_shum', mrg_type='copy')
       end if
    end if
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Sa_shum_wiso')
          call addfldTo(complnd, 'Sa_shum_wiso')
       else
          if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Sa_shum_wiso', rc=rc) .and. &
             ...
             call addMapFrom(compatm, 'Sa_shum_wiso', complnd, mapbilnr, 'one', atm2lnd_map)
             call addmrgTo(complnd, 'Sa_shum_wiso', mrg_from=compatm, mrg_fld='Sa_shum_wiso', mrg_type='copy')
          end if
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_rainc')
       call addfldTo(complnd, 'Faxa_rainc')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_rainc', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_rainc', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_rainc', mrg_from=compatm, mrg_fld='Faxa_rainc', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_rainl')
       call addfldTo(complnd, 'Faxa_rainl')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_rainl', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_rainl', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_rainl', mrg_from=compatm, mrg_fld='Faxa_rainl', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_snowc')
       call addfldTo(complnd, 'Faxa_snowc')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_snowc', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_snowc', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_snowc', mrg_from=compatm, mrg_fld='Faxa_snowc', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_snowl')
       call addfldTo(complnd, 'Faxa_snowl')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_snowl', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_snowl', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_snowl', mrg_from=compatm, mrg_fld='Faxa_snowl', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_lwdn')
       call addfldTo(complnd, 'Faxa_lwdn')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_lwdn', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Faxa_lwdn', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_lwdn', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_lwdn', mrg_from=compatm, mrg_fld='Faxa_lwdn', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndr')
       call addfldTo(complnd, 'Faxa_swndr')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_swndr', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_swndr', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_swndr', mrg_from=compatm, mrg_fld='Faxa_swndr', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdr')
       call addfldTo(complnd, 'Faxa_swvdr')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_swvdr', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_swvdr', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_swvdr', mrg_from=compatm, mrg_fld='Faxa_swvdr', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndf')
       call addfldTo(complnd, 'Faxa_swndf')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_swndf', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_swndf', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_swndf', mrg_from=compatm, mrg_fld='Faxa_swndf', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdf')
       call addfldTo(complnd, 'Faxa_swvdf')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_swvdf', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_swvdf', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_swvdf', mrg_from=compatm, mrg_fld='Faxa_swvdf', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_bcph')
       call addfldTo(complnd, 'Faxa_bcph')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_bcph', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Faxa_bcph', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_bcph', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_bcph', mrg_from=compatm, mrg_fld='Faxa_bcph', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_ocph')
       call addfldTo(complnd, 'Faxa_ocph')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_ocph', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Faxa_ocph', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_ocph', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_ocph', mrg_from=compatm, mrg_fld='Faxa_ocph', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       ...
       call addfldTo(complnd, 'Faxa_dstwet')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_dstwet', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_dstwet', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_dstwet', mrg_from=compatm, mrg_fld='Faxa_dstwet', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_dstdry')
       call addfldTo(complnd, 'Faxa_dstdry')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_dstdry', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_dstdry', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_dstdry', mrg_from=compatm, mrg_fld='Faxa_dstdry', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_ndep')
       call addfldTo(complnd, 'Faxa_ndep')
    else
       if ( fldchk(is_local%wrap%FBexp(complnd)         , 'Faxa_ndep', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Faxa_ndep', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_ndep', complnd, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(complnd, 'Faxa_ndep', mrg_from=compatm, mrg_fld='Faxa_ndep', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Flrr_volr')
       call addfldTo(complnd, 'Flrr_volr')
    else
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_volr', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(comprof, comprof), 'Flrr_volr', rc=rc)) then
          call addmapFrom(comprof, 'Flrr_volr', complnd, mapconsf, 'one', rof2lnd_map)
          call addmrgTo(complnd, 'Flrr_volr', mrg_from=comprof, mrg_fld='Flrr_volr', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Flrr_volrmch')
       call addfldTo(complnd, 'Flrr_volrmch')
    else
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_volrmch', rc=rc) .and. &
          ...
          call addmapFrom(comprof, 'Flrr_volrmch', complnd, mapconsf, 'one', rof2lnd_map)
          call addmrgTo(complnd, 'Flrr_volrmch', mrg_from=comprof, mrg_fld='Flrr_volrmch', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Flrr_flood')
       call addfldTo(complnd, 'Flrr_flood')
    else
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_flood', rc=rc) .and. &
          ...
          call addmapFrom(comprof, 'Flrr_flood', complnd, mapconsf, 'one', rof2lnd_map)
          call addmrgTo(complnd, 'Flrr_flood', mrg_from=comprof, mrg_fld='Flrr_flood', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Sr_tdepth')
       call addfldTo(complnd, 'Sr_tdepth')
    else
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Sr_tdepth', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(comprof, comprof), 'Sr_tdepth', rc=rc)) then
          call addmapFrom(comprof, 'Sr_tdepth', complnd, mapconsf, 'one', rof2lnd_map)
          call addmrgTo(complnd, 'Sr_tdepth', mrg_from=comprof, mrg_fld='Sr_tdepth', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Sr_tdepth_max')
       call addfldTo(complnd, 'Sr_tdepth_max')
    else
       if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Sr_tdepth_max', rc=rc) .and. &
          ...
          call addmapFrom(comprof, 'Sr_tdepth_max', complnd, mapconsf, 'one', rof2lnd_map)
          call addmrgTo(complnd, 'Sr_tdepth_max', mrg_from=comprof, mrg_fld='Sr_tdepth_max', mrg_type='copy')
       end if
    end if

    if (flds_wiso) then
       if (phase == 'advertise') then
          call addfldFrom(comprof, 'Flrr_volr_wiso')
          call addfldTo(complnd, 'Flrr_volr_wiso')
       else
          if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_volr_wiso', rc=rc) .and. &
             ...
             call addmapFrom(comprof, 'Flrr_volr_wiso', complnd, mapconsf, 'one', rof2lnd_map)
             call addmrgTo(complnd, 'Flrr_volr_wiso', &
                  mrg_from=comprof, mrg_fld='Flrr_volr_wiso', mrg_type='copy')
          end if
       end if
       if (phase == 'advertise') then
          call addfldFrom(comprof, 'Flrr_volrmch_wiso')
          ...
          if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_volrmch_wiso', rc=rc) .and. &
             ...
             call addmapFrom(comprof, 'Flrr_volrmch_wiso', complnd, mapconsf, 'one', rof2lnd_map)
             call addmrgTo(complnd, 'Flrr_volrmch_wiso', &
                  mrg_from=comprof, mrg_fld='Flrr_volrmch_wiso', mrg_type='copy')
          end if
       end if
       if (phase == 'advertise') then
          call addfldFrom(comprof, 'Flrr_flood_wiso')
          call addfldTo(complnd, 'Flrr_flood_wiso')
       else
          if ( fldchk(is_local%wrap%FBExp(complnd)         , 'Flrr_flood_wiso', rc=rc) .and. &
             ...
             call addmapFrom(comprof, 'Flrr_flood_wiso', complnd, mapconsf, 'one', rof2lnd_map)
             call addmrgTo(complnd, 'Flrr_flood_wiso', &
    ...
    if (phase == 'advertise') then
       do ns = 1, is_local%wrap%num_icesheets
          call addfldFrom(compglc(ns), 'Sg_icemask')     ! ice sheet grid coverage
          call addfldFrom(compglc(ns), 'Sg_icemask_coupled_fluxes')
          call addfldFrom(compglc(ns), 'Sg_ice_covered') ! fraction of glacier area
          call addfldFrom(compglc(ns), 'Sg_topo')        ! surface height of glacer
          call addfldFrom(compglc(ns), 'Flgg_hflx')      ! downward heat flux from glacier interior
       end do
       call addfldTo(complnd, 'Sg_icemask')
       call addfldTo(complnd, 'Sg_icemask_coupled_fluxes')
       call addfldTo(complnd, 'Sg_ice_covered_elev')
       call addfldTo(complnd, 'Sg_topo_elev')
       call addfldTo(complnd, 'Flgg_hflx_elev')
    else
       ...
       if ( fldchk(is_local%wrap%FBExp(complnd), 'Sg_icemask', rc=rc)) then
          do ns = 1, is_local%wrap%num_icesheets
             if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Sg_icemask', rc=rc)) then
                call addmapFrom(compglc(ns), 'Sg_icemask', &
       ...
       if ( fldchk(is_local%wrap%FBExp(complnd), 'Sg_icemask_coupled_fluxes', rc=rc)) then
          do ns = 1, is_local%wrap%num_icesheets
             if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Sg_icemask_coupled_fluxes', rc=rc)) then
                call addmapFrom(compglc(ns), 'Sg_icemask_coupled_fluxes', &
                     complnd, mapconsd, 'one', 'unset')
             end if
          end do
       end if
    ...
    if (phase == 'advertise') then
       ...
       call addfldTo(compatm, 'Sl_lfrac')
       call addfldTo(compatm, 'Si_ifrac')
       call addfldTo(compatm, 'So_ofrac')
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_avsdr')
       call addfldFrom(compice, 'Si_avsdr')
       call addocnalbFld('So_avsdr')
       call addfldTo(compatm, 'Sx_avsdr')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_avsdr', rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(complnd,complnd), 'Sl_avsdr', rc=rc)) then
             call addmapFrom(complnd, 'Sl_avsdr', compatm, mapconsf, 'lfrin', lnd2atm_map)
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Si_avsdr', rc=rc)) then
             call addMapFrom(compice, 'Si_avsdr', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm, 'Sx_avsdr', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_avsdf')
       call addfldFrom(compice, 'Si_avsdf')
       call addocnalbFld( 'So_avsdf')
       call addfldTo(compatm, 'Sx_avsdf')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_avsdf', rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(complnd,complnd), 'Sl_avsdf', rc=rc)) then
             call addmapFrom(complnd, 'Sl_avsdf', compatm, mapconsf, 'lfrin', lnd2atm_map)
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Si_avsdf', rc=rc)) then
             call addMapFrom(compice, 'Si_avsdf', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm, 'Sx_avsdf', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_anidr')
       call addfldFrom(compice, 'Si_anidr')
       call addocnalbFld( 'So_anidr')
       call addfldTo(compatm, 'Sx_anidr')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_anidr', rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(complnd,complnd), 'Sl_anidr', rc=rc)) then
             call addmapFrom(complnd, 'Sl_anidr', compatm, mapconsf, 'lfrin', lnd2atm_map)
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Si_anidr', rc=rc)) then
             call addMapFrom(compice, 'Si_anidr', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm, 'Sx_anidr', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_anidf')
       call addfldFrom(compice, 'Si_anidf')
       call addocnalbFld( 'So_anidf')
       call addfldTo(compatm, 'Sx_anidf')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_anidf', rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(complnd,complnd), 'Sl_anidf', rc=rc)) then
             call addmapFrom(complnd, 'Sl_anidf', compatm, mapconsf, 'lfrin', lnd2atm_map)
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Si_anidf', rc=rc)) then
             call addMapFrom(compice, 'Si_anidf', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm, 'Sx_anidf', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd , 'Sl_tref')
       call addfldFrom(compice , 'Si_tref')
       call addaofluxFld('So_tref')
       call addfldTo(compatm , 'Sx_tref')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_tref', rc=rc)) then
          if (fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_tref', rc=rc)) then
             call addmapFrom(complnd , 'Sl_tref', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Sx_tref', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_tref', rc=rc)) then
             call addMapFrom(compice , 'Si_tref', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Sx_tref', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_tref', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('So_tref', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Sx_tref', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd , 'Sl_u10')
       call addfldFrom(compice , 'Si_u10')
       call addaofluxFld('So_u10')
       call addfldTo(compatm , 'Sx_u10')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_u10', rc=rc)) then
          if (fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_u10', rc=rc)) then
             call addmapFrom(complnd , 'Sl_u10', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Sx_u10', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_u10', rc=rc)) then
             call addMapFrom(compice , 'Si_u10', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Sx_u10', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_u10', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('So_u10', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Sx_u10', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd , 'Sl_qref')
       call addfldFrom(compice , 'Si_qref')
       call addaofluxFld('So_qref')
       call addfldTo(compatm , 'Sx_qref')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_qref', rc=rc)) then
          if (fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_qref', rc=rc)) then
             call addmapFrom(complnd , 'Sl_qref', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Sx_qref', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_qref', rc=rc)) then
             call addMapFrom(compice , 'Si_qref', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Sx_qref', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_qref', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('So_qref', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Sx_qref', &
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addFldFrom(complnd , 'Sl_qref_wiso')
          call addfldFrom(compice , 'Si_qref_wiso')
          call addaofluxFld('So_qref_wiso')
          call addfldTo(compatm , 'Sx_qref_wiso')
       else
          if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_qref_wiso', rc=rc)) then
             if (fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_qref_wiso', rc=rc)) then
                call addmapFrom(complnd , 'Sl_qref_wiso', compatm, mapconsf, 'lfrin', lnd2atm_map)
                call addmrgTo(compatm , 'Sx_qref_wiso', &
             ...
             if (fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_qref_wiso', rc=rc)) then
                call addMapFrom(compice , 'Si_qref_wiso', compatm, mapconsf, 'ifrac', ice2atm_map)
                call addmrgTo(compatm , 'Sx_qref_wiso', &
             ...
             if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_qref_wiso', rc=rc)) then
                if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                   call addaofluxmap( 'So_qref_wiso', compatm, mapconsf, 'ofrac', ocn2atm_map) ! map ocn->atm
                end if
                call addmrgTo(compatm , 'Sx_qref_wiso', &
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compatm), 'Sx_qref_wiso', rc=rc)) then
             ...
             if (fldchk(is_local%wrap%FBMed_aoflux_o, 'So_qref_wiso', rc=rc)) then
                if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                   call addaofluxmap('So_qref_wiso', compatm, mapconsf, 'ofrac', ocn2atm_map)
                end if
                call addmrgTo(compatm , 'Sx_qref_wiso', &
                     mrg_from=compmed, mrg_fld='So_qref_wiso', mrg_type='merge', mrg_fracname='ofrac')
             end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldTo(compatm, 'Faxx_taux')
       call addFldFrom(complnd, 'Fall_taux')
       call addfldFrom(compice, 'Faii_taux')
       call addaofluxFld( 'Faox_taux')
    else
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_taux', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_taux', rc=rc)) then
             call addmapFrom(complnd , 'Fall_taux', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_taux', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_taux', rc=rc)) then
             call addMapFrom(compice , 'Faii_taux', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_taux', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_taux', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_taux', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Faxx_taux', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_tauy', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_tauy', rc=rc)) then
             call addmapFrom(complnd , 'Fall_tauy', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_tauy', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_tauy', rc=rc)) then
             call addMapFrom(compice , 'Faii_tauy', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_tauy', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_tauy', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_tauy', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Faxx_tauy', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_lat', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_lat', rc=rc)) then
             call addmapFrom(complnd , 'Fall_lat', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_lat', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_lat', rc=rc)) then
             call addMapFrom(compice , 'Faii_lat', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_lat', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_lat', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_lat', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Faxx_lat', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_sen', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_sen', rc=rc)) then
             call addmapFrom(complnd , 'Fall_sen', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_sen', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_sen', rc=rc)) then
             call addMapFrom(compice , 'Faii_sen', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_sen', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_sen', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_sen', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Faxx_sen', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_evap', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_evap', rc=rc)) then
             call addmapFrom(complnd , 'Fall_evap', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_evap', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_evap', rc=rc)) then
             call addMapFrom(compice , 'Faii_evap', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_evap', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_evap', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_evap', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm , 'Faxx_evap', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_lwup', rc=rc)) then
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_lwup', rc=rc)) then
             call addmapFrom(complnd , 'Fall_lwup', compatm, mapconsf, 'lfrin', lnd2atm_map)
             call addmrgTo(compatm , 'Faxx_lwup', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_lwup', rc=rc)) then
             call addMapFrom(compice , 'Faii_lwup', compatm, mapconsf, 'ifrac', ice2atm_map)
             call addmrgTo(compatm , 'Faxx_lwup', &
          ...
          if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_lwup', rc=rc)) then
             if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                call addaofluxmap('Faox_lwup', compatm, mapconsf, 'ofrac', ocn2atm_map)
             end if
             call addmrgTo(compatm, 'Faxx_lwup', &
       ...
       if (phase == 'advertise') then
          ...
          if (fldchk(is_local%wrap%FBexp(compatm), 'Faxx_evap_wiso', rc=rc)) then
             if ( fldchk(is_local%wrap%FBImp(complnd,complnd), 'Fall_evap_wiso', rc=rc)) then
                call addmapFrom(complnd , 'Fall_evap_wiso', compatm, mapconsf, 'lfrin', lnd2atm_map)
                call addmrgTo(compatm , 'Faxx_evap_wiso', &
             ...
             if (fldchk(is_local%wrap%FBImp(compice,compice), 'Faii_evap_wiso', rc=rc)) then
                call addMapFrom(compice , 'Faii_evap_wiso', compatm, mapconsf, 'ifrac', ice2atm_map)
                call addmrgTo(compatm , 'Faxx_evap_wiso', &
             ...
             if (fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_evap_wiso', rc=rc)) then
                if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
                   call addaofluxmap('Faox_evap_wiso', compatm, mapconsf, 'ofrac', ocn2atm_map)
                end if
                call addmrgTo(compatm , 'Faxx_evap_wiso', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_t')
       call addfldFrom(compice, 'Si_t')
       call addfldFrom(compocn, 'So_t')
       call addfldTo(compatm, 'So_t')
       call addfldTo(compatm, 'Sx_t')
    else
       if (fldchk(is_local%wrap%FBexp(compatm), 'Sx_t', rc=rc)) then
          if (fldchk(is_local%wrap%FBImp(complnd,complnd), 'Sl_t', rc=rc)) then
             call addmapFrom(complnd, 'Sl_t', compatm, mapconsf , 'lfrin', lnd2atm_map)
             call addmrgTo(compatm, 'Sx_t', &
          ...
          if (fldchk(is_local%wrap%FBImp(compice,compice), 'Si_t', rc=rc)) then
             call addMapFrom(compice, 'Si_t', compatm, mapconsf , 'ifrac', ice2atm_map)
             call addmrgTo(compatm, 'Sx_t', &
          ...
          if (fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_t', rc=rc)) then
             call addmapFrom(compocn, 'So_t', compatm, mapconsf, 'ofrac', ocn2atm_map)
             call addmrgTo(compatm, 'Sx_t', &
                  mrg_from=compocn, mrg_fld='So_t', mrg_type='merge', mrg_fracname='ofrac')
          end if
       end if
       if (fldchk(is_local%wrap%FBexp(compatm), 'So_t', rc=rc)) then
          call addmrgTo(compatm, 'So_t', mrg_from=compocn, mrg_fld='So_t', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Si_snowh')
       call addfldTo(compatm, 'Si_snowh')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)        , 'Si_snowh', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compice,compice), 'Si_snowh', rc=rc)) then
          call addMapFrom(compice, 'Si_snowh', compatm, mapconsf, 'ifrac', ice2atm_map)
          call addmrgTo(compatm, 'Si_snowh', mrg_from=compice, mrg_fld='Si_snowh', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Si_vice')
       call addfldTo(compatm, 'Si_vice')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)        , 'Si_vice', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compice,compice), 'Si_vice', rc=rc)) then
          call addMapFrom(compice, 'Si_vice', compatm, mapconsf, 'ifrac', ice2atm_map)
          call addmrgTo(compatm, 'Si_vice', mrg_from=compice, mrg_fld='Si_vice', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Si_vsno')
       call addfldTo(compatm, 'Si_vsno')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)        , 'Si_vsno', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compice,compice), 'Si_vsno', rc=rc)) then
          call addMapFrom(compice, 'Si_vsno', compatm, mapconsf, 'ifrac', ice2atm_map)
          call addmrgTo(compatm, 'Si_vsno', mrg_from=compice, mrg_fld='Si_vsno', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addaofluxFld('So_ssq')
       call addfldTo(compatm , 'So_ssq')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm) , 'So_ssq', rc=rc) .and. &
            fldchk(is_local%wrap%FBMed_aoflux_o , 'So_ssq', rc=rc)) then
          if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
             call addaofluxmap( 'So_ssq', compatm, mapconsf, 'ofrac', ocn2atm_map)
          end if
          call addmrgTo(compatm   , 'So_ssq', mrg_from=compmed, mrg_fld='So_ssq', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addaofluxFld('So_re')
       call addfldTo(compatm , 'So_re')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm) , 'So_re', rc=rc) .and. &
            fldchk(is_local%wrap%FBMed_aoflux_o , 'So_re', rc=rc)) then
          if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
             call addaofluxmap( 'So_re', compatm, mapconsf, 'ofrac', ocn2atm_map)
          end if
          call addmrgTo(compatm   , 'So_re', mrg_from=compmed, mrg_fld='So_re', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addaofluxFld('So_ustar')
       call addfldTo(compatm , 'So_ustar')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm) , 'So_ustar', rc=rc) .and. &
            fldchk(is_local%wrap%FBMed_aoflux_o , 'So_ustar', rc=rc)) then
          if (trim(is_local%wrap%aoflux_grid) == 'ogrid') then
             call addaofluxmap( 'So_ustar', compatm, mapconsf, 'ofrac', ocn2atm_map)
          end if
          call addmrgTo(compatm   , 'So_ustar', mrg_from=compmed, mrg_fld='So_ustar', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_fv')
       call addfldTo(compatm, 'Sl_fv')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_fv', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_fv', rc=rc)) then
          call addmapFrom(complnd, 'Sl_fv', compatm, mapconsf, 'lfrin', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_fv', mrg_from=complnd, mrg_fld='Sl_fv', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_ram1')
       call addfldTo(compatm, 'Sl_ram1')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_ram1', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_ram1', rc=rc)) then
          call addmapFrom(complnd, 'Sl_ram1', compatm, mapconsf, 'lfrin', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_ram1', mrg_from=complnd, mrg_fld='Sl_ram1', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_snowh')
       call addfldTo(compatm, 'Sl_snowh')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_snowh', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_snowh', rc=rc)) then
          call addmapFrom(complnd, 'Sl_snowh', compatm, mapconsf, 'lfrin', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_snowh', mrg_from=complnd, mrg_fld='Sl_snowh', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_soilw')
       call addfldTo(compatm, 'Sl_soilw')
    else
       if ( fldchk(is_local%wrap%FBexp(compatm)         , 'Sl_soilw', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(complnd,complnd ), 'Sl_soilw', rc=rc)) then
          call addmapFrom(complnd, 'Sl_soilw', compatm, mapconsf, 'lfrin', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_soilw', mrg_from=complnd, mrg_fld='Sl_soilw', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Fall_flxdst')
       call addfldTo(compatm, 'Fall_flxdst')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Fall_flxdst', rc=rc) .and. &
          ...
          call addmapFrom(complnd, 'Fall_flxdst', compatm, mapconsf, 'lfrin', lnd2atm_map)
          call addmrgTo(compatm, 'Fall_flxdst', &
    ...
    !-----------------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Fall_voc')
       call addfldTo(compatm, 'Fall_voc')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Fall_voc', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compatm)         , 'Fall_voc', rc=rc)) then
          call addmapFrom(complnd, 'Fall_voc', compatm, mapconsf, 'one', atm2lnd_map)
          call addmrgTo(compatm, 'Fall_voc', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Fall_fire')
       call addfldTo(compatm, 'Fall_fire')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Fall_fire', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compatm)         , 'Fall_fire', rc=rc)) then
          call addmapFrom(complnd, 'Fall_fire', compatm, mapconsf, 'one', lnd2atm_map)
          call addmrgTo(compatm, 'Fall_fire', &
               mrg_from=complnd, mrg_fld='Fall_fire', mrg_type='merge', mrg_fracname='lfrac')
       end if
    end if
    ! 'wild fire plume height'
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_fztop')
       call addfldTo(compatm, 'Sl_fztop')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Sl_fztop', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compatm)         , 'Sl_fztop', rc=rc)) then
          call addmapFrom(complnd, 'Sl_fztop', compatm, mapconsf, 'one', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_fztop', mrg_from=complnd, mrg_fld='Sl_fztop', mrg_type='copy')
       end if
    end if
    !-----------------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_ddvel')
       call addfldTo(compatm, 'Sl_ddvel')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Sl_ddvel', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compatm)         , 'Sl_ddvel', rc=rc)) then
          call addmapFrom(complnd, 'Sl_ddvel', compatm, mapconsf, 'one', lnd2atm_map)
          call addmrgTo(compatm, 'Sl_ddvel', mrg_from=complnd, mrg_fld='Sl_ddvel', mrg_type='copy')
       end if
    end if
    ...
    !----------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Si_ifrac')
       call addFldTo(compocn, 'Si_ifrac')
    else
       call addMapFrom(compice, 'Si_ifrac', compocn,  mapfcopy, 'unset', 'unset')
       call addmrgTo(compocn, 'Si_ifrac', mrg_from=compice, mrg_fld='Si_ifrac', mrg_type='copy')
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_lwdn')
       call addFldTo(compocn, 'Faxa_lwdn')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_lwdn', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_lwdn', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_lwdn', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_lwdn', &
               mrg_from=compatm, mrg_fld='Faxa_lwdn', mrg_type='copy_with_weights', mrg_fracname='ofrac')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndr')
       call addFldTo(compocn, 'Faxa_swndr')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_swndr', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swndr', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swndr', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_swndr', &
               mrg_from=compatm, mrg_fld='Faxa_swndr', mrg_type='copy_with_weights', mrg_fracname='ofrac')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndf')
       call addFldTo(compocn, 'Faxa_swndf')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_swndf', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swndf', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swndf', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_swndf', &
               mrg_from=compatm, mrg_fld='Faxa_swndf', mrg_type='copy_with_weights', mrg_fracname='ofrac')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdr')
       call addFldTo(compocn, 'Faxa_swvdr')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_swvdr', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swvdr', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swvdr', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_swvdr', &
               mrg_from=compatm, mrg_fld='Faxa_swvdr', mrg_type='copy_with_weights', mrg_fracname='ofrac')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdf')
       call addFldTo(compocn, 'Faxa_swvdf')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_swvdf', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swvdf', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swvdf', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_swvdf', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm , 'Faxa_lwdn')
       call addaofluxFld('Faox_lwup' )
       call addFldTo(compocn , 'Foxx_lwnet')
    else
       ...
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Foxx_lwnet', rc=rc) .and. &
            ...
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_lwdn' , rc=rc)) then
          call addMapFrom(compatm, 'Faxa_lwdn', compocn, mapconsf, 'one'  , atm2ocn_map)
          call addmrgTo(compocn, 'Foxx_lwnet', &
               mrg_from=compmed, mrg_fld='Faox_lwup', mrg_type='merge', mrg_fracname='ofrac')
          call addmrgTo(compocn, 'Foxx_lwnet', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swdn')
       call addFldTo(compocn, 'Faxa_swdn')
    else
       if (fldchk(is_local%wrap%FBImp(compatm, compatm), 'Faxa_swdn', rc=rc) .and. &
           fldchk(is_local%wrap%FBExp(compocn)         , 'Faxa_swdn', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swdn', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_swdn', &
    ...
    if (phase == 'advertise') then
       ...
       call addfldFrom(compice, 'Fioi_swpen')
       call addfldFrom(compice, 'Fioi_swpen_vdr')
       call addfldFrom(compice, 'Fioi_swpen_vdf')
       call addfldFrom(compice, 'Fioi_swpen_idr')
       call addfldFrom(compice, 'Fioi_swpen_idf')

       call addFldTo(compocn, 'Foxx_swnet')
       call addFldTo(compocn, 'Foxx_swnet_vdr')
       call addFldTo(compocn, 'Foxx_swnet_vdf')
       call addFldTo(compocn, 'Foxx_swnet_idr')
       call addFldTo(compocn, 'Foxx_swnet_idf')
    else
       ...
       if (fldchk(is_local%wrap%FBImp(compice,compice), 'Fioi_swpen', rc=rc)) then
          call addMapFrom(compice, 'Fioi_swpen',  compocn, mapfcopy, 'unset', 'unset')
       end if
       ...
       if ( fldchk(is_local%wrap%FBImp(compice,compice), 'Fioi_swpen_vdr', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_swpen_vdr', compocn, mapfcopy, 'unset', 'unset')
          call addMapFrom(compice, 'Fioi_swpen_vdf', compocn, mapfcopy, 'unset', 'unset')
          call addMapFrom(compice, 'Fioi_swpen_idr', compocn, mapfcopy, 'unset', 'unset')
          call addMapFrom(compice, 'Fioi_swpen_idf', compocn, mapfcopy, 'unset', 'unset')
       end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_rainc')
       call addFldFrom(compatm, 'Faxa_rainl')
       call addFldTo(compocn, 'Faxa_rain' )
       call addFldFrom(compatm, 'Faxa_snowc')
       call addFldFrom(compatm, 'Faxa_snowl')
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_rainl', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_rainl', compocn, mapconsf, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Faxa_rainc', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_rain' , mrg_from=compatm, mrg_fld='Faxa_rainc:Faxa_rainl', &
       ...
       if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_snow' , rc=rc) .and. &
            ...
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snowc', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_snowl', compocn, mapconsf, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Faxa_snowc', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_snow'  , &
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Faxa_rainc_wiso')
          call addFldFrom(compatm, 'Faxa_rainl_wiso')
          call addFldTo(compocn, 'Faxa_rain_wiso' )
          call addFldFrom(compatm, 'Faxa_snowc_wiso')
          call addFldFrom(compatm, 'Faxa_snowl_wiso')
          call addFldFrom(compatm, 'Faxa_snow_wiso' )
       else
          ! Note that the mediator atm/ocn flux calculation needs Faxa_rainc for the gustiness parameterization
          ! which by default is not actually used
          if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_rainl_wiso', rc=rc) .and. &
             ...
             call addMapFrom(compatm, 'Faxa_rainl_wiso', compocn, mapconsf, 'one', atm2ocn_map)
             call addMapFrom(compatm, 'Faxa_rainc_wiso', compocn, mapconsf, 'one', atm2ocn_map)
             call addmrgTo(compocn, 'Faxa_rain_wiso' , &
          ...
          if ( fldchk(is_local%wrap%FBExp(compocn)        , 'Faxa_snow_wiso', rc=rc) .and. &
             ...
             call addMapFrom(compatm, 'Faxa_snowl_wiso', compocn, mapconsf, 'one', atm2ocn_map)
             call addMapFrom(compatm, 'Faxa_snowc_wiso', compocn, mapconsf, 'one', atm2ocn_map)
             call addmrgTo(compocn, 'Faxa_snow_wiso', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm , 'Faxa_sen')
       call addaofluxFld('Faox_sen')
       call addfldFrom(compice , 'Fioi_melth')
       call addFldTo(compocn , 'Foxx_sen')
    else
       if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_sen', rc=rc) .and. &
            fldchk(is_local%wrap%FBMed_aoflux_o, 'Faox_sen', rc=rc)) then
          call addmrgTo(compocn, 'Foxx_sen', &
               mrg_from=compmed, mrg_fld='Faox_sen', mrg_type='merge', mrg_fracname='ofrac')
       end if
    end if
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_lat', rc=rc)) then
          call addmrgTo(compocn, 'Foxx_lat', &
               mrg_from=compmed, mrg_fld='Faox_lat', mrg_type='merge', mrg_fracname='ofrac')
       end if
       if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_evap', rc=rc)) then
          call addmrgTo(compocn, 'Foxx_evap', &
               mrg_from=compmed, mrg_fld='Faox_evap', mrg_type='merge', mrg_fracname='ofrac')
       end if
    end if
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_lat_wiso', rc=rc)) then
             call addmrgTo(compocn, 'Foxx_lat_wiso', &
    ...
    if (phase == 'advertise') then
       ...
       if (fldchk(is_local%wrap%FBExp(compocn), 'So_duu10n', rc=rc)) then
          call addmrgTo(compocn, 'So_duu10n', mrg_from=compmed, mrg_fld='So_duu10n', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_pslv')
       call addFldTo(compocn, 'Sa_pslv')
    else
       if ( fldchk(is_local%wrap%FBImp(compatm, compatm), 'Sa_pslv', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compocn)         , 'Sa_pslv', rc=rc)) then
          call addMapFrom(compatm, 'Sa_pslv', compocn, mapbilnr, 'one', atm2ocn_map)
          call addMapFrom(compatm, 'Sa_pslv', compice, mapbilnr, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Sa_pslv', &
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_dstwet', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_dstwet', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_dstwet', &
               mrg_from=compatm, mrg_fld='Faxa_dstwet', mrg_type='copy_with_weights', mrg_fracname='ofrac')
       end if
    end if
    if (phase == 'advertise') then
       call addFldTo(compocn, 'Faxa_dstdry')
       call addFldFrom(compatm, 'Faxa_dstdry')
    else
       if ( fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_dstdry', rc=rc) .and. &
          ...
          call addMapFrom(compatm, 'Faxa_dstdry', compocn, mapconsf, 'one', atm2ocn_map)
          call addmrgTo(compocn, 'Faxa_dstdry', &
    ...
    if (phase == 'advertise') then
       call addFldTo(compocn, 'Foxx_hrain')
       call addFldTo(compocn, 'Foxx_hsnow')
       call addFldTo(compocn, 'Foxx_hevap')
       call addFldTo(compocn, 'Foxx_hcond')
       call addFldTo(compocn, 'Foxx_hrofl')
       call addFldTo(compocn, 'Foxx_hrofi')
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldTo(compocn , 'Foxx_taux')
       call addfldFrom(compice , 'Fioi_taux')
       call addaofluxFld('Faox_taux')
    else
       if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_taux', rc=rc)) then
          if (fldchk(is_local%wrap%FBimp(compice,compice), 'Fioi_taux', rc=rc)) then
             call addMapFrom(compice, 'Fioi_taux', compocn, mapfcopy, 'unset', 'unset')
             call addmrgTo(compocn, 'Foxx_taux', &
    ...
    if (phase == 'advertise') then
       call addFldTo(compocn , 'Foxx_tauy')
       call addfldFrom(compice , 'Fioi_tauy')
       call addaofluxFld('Faox_tauy')
    else
       if ( fldchk(is_local%wrap%FBexp(compocn), 'Foxx_tauy', rc=rc)) then
          if (fldchk(is_local%wrap%FBimp(compice,compice), 'Fioi_tauy', rc=rc)) then
             call addMapFrom(compice, 'Fioi_tauy', compocn, mapfcopy, 'unset', 'unset')
             call addmrgTo(compocn, 'Foxx_tauy', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice , 'Fioi_meltw')
       call addFldTo(compocn , 'Fioi_meltw')
    else
       if ( fldchk(is_local%wrap%FBexp(compocn)         , 'Fioi_meltw', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_meltw',    compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_meltw', &
               mrg_from=compice, mrg_fld='Fioi_meltw', mrg_type='copy_with_weights', mrg_fracname='ifrac')
       end if
    end if
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addfldFrom(compice , 'Fioi_meltw_wiso')
          call addFldTo(compocn , 'Fioi_meltw_wiso')
       else
          if ( fldchk(is_local%wrap%FBexp(compocn)         , 'Fioi_meltw_wiso', rc=rc) .and. &
             ...
             call addMapFrom(compice, 'Fioi_meltw_wiso',    compocn,  mapfcopy, 'unset', 'unset')
             call addmrgTo(compocn, 'Fioi_meltw_wiso', &
    ...
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Fioi_melth')
       call addFldTo(compocn, 'Fioi_melth')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Fioi_melth', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_melth', compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_melth', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Fioi_salt')
       call addFldTo(compocn, 'Fioi_salt')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Fioi_salt', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compice, compice), 'Fioi_salt', rc=rc)) then
          call addMapFrom(compice, 'Fioi_salt', compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_salt', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Fioi_bcphi')
       call addFldTo(compocn, 'Fioi_bcphi')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Fioi_bcphi', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_bcphi', compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_bcphi', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Fioi_bcpho')
       call addFldTo(compocn, 'Fioi_bcpho')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Fioi_bcpho', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_bcpho', compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_bcpho', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Fioi_flxdst')
       call addFldTo(compocn, 'Fioi_flxdst')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Fioi_flxdst', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Fioi_flxdst', compocn,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compocn, 'Fioi_flxdst', &
    ...
    if (phase == 'advertise') then
       ...
       do ns = 1, is_local%wrap%num_icesheets
          call addfldFrom(compglc(ns), 'Fogg_rofl')
       end do
       call addfldFrom(comprof, 'Forr_rofl')
       call addFldTo(compocn, 'Foxx_rofl')
       call addFldTo(compocn, 'Flrr_flood')
       do ns = 1, is_local%wrap%num_icesheets
          call addfldFrom(compglc(ns), 'Fogg_rofi')
       end do
       call addfldFrom(comprof, 'Forr_rofi')
       call addFldTo(compocn, 'Foxx_rofi')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_rofl' , rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofl' , rc=rc)) then
             if (trim(rof2ocn_liq_rmap) == 'unset') then
                call addmapFrom(comprof, 'Forr_rofl', compocn, mapconsd, 'none', 'unset')
             else
                call addmapFrom(comprof, 'Forr_rofl', compocn, map_rof2ocn_liq, 'none', rof2ocn_liq_rmap)
             end if
             if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Flrr_flood', rc=rc)) then
                call addmapFrom(comprof, 'Flrr_flood', compocn, mapconsd, 'one', rof2ocn_fmap)
                call addmrgTo(compocn, 'Foxx_rofl', mrg_from=comprof, mrg_fld='Forr_rofl:Flrr_flood', mrg_type='sum')
             else
                call addmrgTo(compocn, 'Foxx_rofl', mrg_from=comprof, mrg_fld='Forr_rofl', mrg_type='sum')
             end if
          end if
          ...
          do ns = 1, is_local%wrap%num_icesheets
             if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Fogg_rofl' , rc=rc)) then
                ...
                call addmapFrom(compglc(ns), 'Fogg_rofl', compocn, map_glc2ocn_liq, 'one' , glc2ocn_liq_rmap)
                call addmrgTo(compocn, 'Foxx_rofl', mrg_from=compglc(ns), mrg_fld='Fogg_rofl', mrg_type='sum')
             end if
          end do
       end if
       if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_rofi' , rc=rc)) then
          ...
          if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofi' , rc=rc)) then
             if (trim(rof2ocn_ice_rmap) == 'unset') then
                call addmapFrom(comprof, 'Forr_rofi', compocn, mapconsd, 'none', 'unset')
             else
                call addmapFrom(comprof, 'Forr_rofi', compocn, map_rof2ocn_ice, 'none', rof2ocn_ice_rmap)
             end if
             call addmrgTo(compocn, 'Foxx_rofi', mrg_from=comprof, mrg_fld='Forr_rofi', mrg_type='sum')
          end if
          ! ice from glc to ocean
          do ns = 1, is_local%wrap%num_icesheets
             if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Fogg_rofi' , rc=rc)) then
                ! TODO: this custom map needs to be different for every ice sheet - how will this be handled?
                call addmapFrom(compglc(ns), 'Fogg_rofi', compocn, map_glc2ocn_ice, 'one', glc2ocn_ice_rmap)
                call addmrgTo(compocn, 'Foxx_rofi', mrg_from=compglc(ns), mrg_fld='Fogg_rofi', mrg_type='sum')
             end if
          end do
       end if
    end if
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          do ns = 1, is_local%wrap%num_icesheets
             call addfldFrom(compglc(ns), 'Fogg_rofl_wiso')
          end do
          call addfldFrom(comprof, 'Forr_rofl_wiso')
          call addFldTo(compocn, 'Foxx_rofl_wiso')
          call addFldTo(compocn, 'Flrr_flood_wiso')
          do ns = 1, is_local%wrap%num_icesheets
             call addfldFrom(compglc(ns), 'Fogg_rofi_wiso')
          end do
          call addfldFrom(comprof, 'Forr_rofi_wiso')
          call addFldTo(compocn, 'Foxx_rofi_wiso')
       else
          if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_rofl_wiso' , rc=rc)) then
             ...
             if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofl_wiso' , rc=rc)) then
                if (trim(rof2ocn_liq_rmap) == 'unset') then
                   call addmapFrom(comprof, 'Forr_rofl_wiso', compocn, mapconsd, 'none', 'unset')
                else
                   call addmapFrom(comprof, 'Forr_rofl_wiso', compocn, map_rof2ocn_liq, 'none', rof2ocn_liq_rmap)
                end if
                if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Flrr_flood_wiso', rc=rc)) then
                   call addmapFrom(comprof, 'Flrr_flood_wiso', compocn, mapconsd, 'one', rof2ocn_fmap)
                   call addmrgTo(compocn, 'Foxx_rofl_wiso', &
             ...
             do ns = 1, is_local%wrap%num_icesheets
                if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Fogg_rofl_wiso' , rc=rc)) then
                   ! TODO: this custom map needs to be different for every ice sheet - how will this be handled?
                   call addmapFrom(compglc(ns), 'Fogg_rofl_wiso', compocn, map_glc2ocn_liq, 'one' , glc2ocn_liq_rmap)
                   call addmrgTo(compocn, 'Foxx_rofl_wiso', &
          ...
          if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_rofi_wiso' , rc=rc)) then
             ...
             if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofi_wiso' , rc=rc)) then
                if (trim(rof2ocn_ice_rmap) == 'unset') then
                   call addmapFrom(comprof, 'Forr_rofi_wiso', compocn, mapconsd, 'none', 'unset')
                else
                   call addmapFrom(comprof, 'Forr_rofi_wiso', compocn, map_rof2ocn_ice, 'none', rof2ocn_ice_rmap)
                end if
                call addmrgTo(compocn, 'Foxx_rofi_wiso', mrg_from=comprof, mrg_fld='Forr_rofi', mrg_type='sum')
             end if
             ! ice from glc to ocean
             do ns = 1, is_local%wrap%num_icesheets
                if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Fogg_rofi_wiso' , rc=rc)) then
                   ! TODO: this custom map needs to be different for every ice sheet - how will this be handled?
                   call addmapFrom(compglc(ns), 'Fogg_rofi_wiso', compocn, map_glc2ocn_ice, 'one', glc2ocn_ice_rmap)
                   call addmrgTo(compocn, 'Foxx_rofi_wiso', &
    ...
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_lamult')
       ...
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_lamult', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_lamult', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_lamult', mrg_from=compwav, mrg_fld='Sw_lamult', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ...
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_ustokes')
       call addFldTo(compocn, 'Sw_ustokes')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_ustokes', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_ustokes', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_ustokes', mrg_from=compwav, mrg_fld='Sw_ustokes', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ...
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_vstokes')
       call addFldTo(compocn, 'Sw_vstokes')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_vstokes', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_vstokes', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_vstokes', mrg_from=compwav, mrg_fld='Sw_vstokes', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ...
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_hstokes')
       call addFldTo(compocn, 'Sw_hstokes')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_hstokes', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_hstokes', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_hstokes', mrg_from=compwav, mrg_fld='Sw_hstokes', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ...
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_pstokes_x')
       call addFldTo(compocn, 'Sw_pstokes_x')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_pstokes_x', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_pstokes_x', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_pstokes_x', mrg_from=compwav, mrg_fld='Sw_pstokes_x', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ! to ocn: Stokes drift depth from wave
    !-----------------------------
    if (phase == 'advertise') then
       call addfldFrom(compwav, 'Sw_pstokes_y')
       call addFldTo(compocn, 'Sw_pstokes_y')
    else
       if ( fldchk(is_local%wrap%FBExp(compocn)         , 'Sw_pstokes_y', rc=rc) .and. &
          ...
          call addmapFrom(compwav, 'Sw_pstokes_y', compocn,  mapbilnr_nstod, 'one', wav2ocn_smap)
          call addmrgTo(compocn, 'Sw_pstokes_y', mrg_from=compwav, mrg_fld='Sw_pstokes_y', mrg_type='copy')
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_lwdn')
       call addfldTo(compice, 'Faxa_lwdn')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_lwdn', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_lwdn', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_lwdn', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_lwdn', mrg_from=compatm, mrg_fld='Faxa_lwdn', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndr')
       call addfldTo(compice, 'Faxa_swndr')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_swndr', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swndr', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swndr', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_swndr', mrg_from=compatm, mrg_fld='Faxa_swndr', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdr')
       call addfldTo(compice, 'Faxa_swvdr')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_swvdr', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swvdr', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swvdr', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_swvdr', mrg_from=compatm, mrg_fld='Faxa_swvdr', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swndf')
       call addfldTo(compice, 'Faxa_swndf')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_swndf', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swndf', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swndf', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_swndf', mrg_from=compatm, mrg_fld='Faxa_swndf', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_swvdf')
       call addfldTo(compice, 'Faxa_swvdf')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_swvdf', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_swvdf', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_swvdf', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_swvdf', mrg_from=compatm, mrg_fld='Faxa_swvdf', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_bcph')
       call addfldTo(compice, 'Faxa_bcph')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_bcph', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_bcph', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_bcph', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_bcph', mrg_from=compatm, mrg_fld='Faxa_bcph', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_ocph')
       call addfldTo(compice, 'Faxa_ocph')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_ocph', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_ocph', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_ocph', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_ocph', mrg_from=compatm, mrg_fld='Faxa_ocph', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       ...
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_dstwet', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_dstwet', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_dstwet', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_dstwet', mrg_from=compatm, mrg_fld='Faxa_dstwet', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_dstdry')
       call addfldTo(compice, 'Faxa_dstdry')
    else
       if ( fldchk(is_local%wrap%FBExp(compice)        , 'Faxa_dstdry', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_dstdry', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_dstdry', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_dstdry', mrg_from=compatm, mrg_fld='Faxa_dstdry', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_rainc')
       call addFldFrom(compatm, 'Faxa_rainl')
       call addFldFrom(compatm, 'Faxa_rain' )
       call addfldTo(compice, 'Faxa_rain' )
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'Faxa_rain' , rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_rainl', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_rainc', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_rainc', compice, mapconsf, 'one', atm2ice_map)
          call addMapFrom(compatm, 'Faxa_rainl', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_rain' , mrg_from=compatm, mrg_fld='Faxa_rainc:Faxa_rainl', mrg_type='sum')
          ...
          call addMapFrom(compatm, 'Faxa_rain', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_rain', mrg_from=compatm, mrg_fld='Faxa_rain', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Faxa_snowc')
       call addFldFrom(compatm, 'Faxa_snowl')
       call addFldFrom(compatm, 'Faxa_snow' )
       call addfldTo(compice, 'Faxa_snow' )
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'Faxa_snow' , rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snowl', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snowc', rc=rc)) then
          call addMapFrom(compatm, 'Faxa_snowc', compice, mapconsf, 'one', atm2ice_map)
          call addMapFrom(compatm, 'Faxa_snowl', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_snow' , &
          ...
          call addMapFrom(compatm, 'Faxa_snow', compice, mapconsf, 'one', atm2ice_map)
          call addmrgTo(compice, 'Faxa_snow', &
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Faxa_rainc_wiso')
          call addFldFrom(compatm, 'Faxa_rainl_wiso')
          call addFldFrom(compatm, 'Faxa_rain_wiso' )
          call addfldTo(compice, 'Faxa_rain_wiso' )
       else
          if ( fldchk(is_local%wrap%FBexp(compice)        , 'Faxa_rain_wiso' , rc=rc) .and. &
             ...
             call addMapFrom(compatm, 'Faxa_rainc_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addMapFrom(compatm, 'Faxa_rainl_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addmrgTo(compice, 'Faxa_rain_wiso' , &
             ...
             call addMapFrom(compatm, 'Faxa_rain_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addmrgTo(compice, 'Faxa_rain_wiso', &
       ...
       if (phase == 'advertise') then
          ...
          call addFldFrom(compatm, 'Faxa_snowl_wiso')
          call addFldFrom(compatm, 'Faxa_snow_wiso' )
          call addfldTo(compice, 'Faxa_snow_wiso' )
       else
          if ( fldchk(is_local%wrap%FBexp(compice)        , 'Faxa_snow_wiso', rc=rc) .and. &
               fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snowl_wiso', rc=rc) .and. &
               fldchk(is_local%wrap%FBImp(compatm,compatm), 'Faxa_snowc_wiso', rc=rc)) then
             call addMapFrom(compatm, 'Faxa_snowc_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addMapFrom(compatm, 'Faxa_snowl_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addmrgTo(compice, 'Faxa_snow_wiso' , &
             ...
             call addMapFrom(compatm, 'Faxa_snow_wiso', compice, mapconsf, 'one', atm2ice_map)
             call addmrgTo(compice, 'Faxa_snow_wiso', mrg_from=compatm, mrg_fld='Faxa_snow_wiso', mrg_type='copy')
          end if
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_z')
       call addfldTo(compice, 'Sa_z')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_z', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_z', rc=rc)) then
          call addMapFrom(compatm, 'Sa_z', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_z', mrg_from=compatm, mrg_fld='Sa_z', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_pbot')
       call addfldTo(compice, 'Sa_pbot')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_pbot', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_pbot', rc=rc)) then
          call addMapFrom(compatm, 'Sa_pbot', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_pbot', mrg_from=compatm, mrg_fld='Sa_pbot', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_tbot')
       call addfldTo(compice, 'Sa_tbot')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_tbot', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_tbot', rc=rc)) then
          call addMapFrom(compatm, 'Sa_tbot', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_tbot', mrg_from=compatm, mrg_fld='Sa_tbot', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_ptem')
       call addfldTo(compice, 'Sa_ptem')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_ptem', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_ptem', rc=rc)) then
          call addMapFrom(compatm, 'Sa_ptem', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_ptem', mrg_from=compatm, mrg_fld='Sa_ptem', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_dens')
       call addfldTo(compice, 'Sa_dens')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_dens', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_dens', rc=rc)) then
          call addMapFrom(compatm, 'Sa_dens', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_dens', mrg_from=compatm, mrg_fld='Sa_dens', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_u')
       call addfldTo(compice, 'Sa_u')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_u', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_u', rc=rc)) then
          if (mapuv_with_cart3d) then
             call addMapFrom(compatm, 'Sa_u', compice, mappatch_uv3d, 'one', atm2ice_map)
          else
             call addMapFrom(compatm, 'Sa_u', compice, mappatch, 'one', atm2ice_map)
          end if
          call addmrgTo(compice, 'Sa_u', mrg_from=compatm, mrg_fld='Sa_u', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_v')
       call addfldTo(compice, 'Sa_v')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_v', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_v', rc=rc)) then
          if (mapuv_with_cart3d) then
             call addMapFrom(compatm, 'Sa_v', compice, mappatch_uv3d, 'one', atm2ice_map)
          else
             call addMapFrom(compatm, 'Sa_v', compice, mappatch, 'one', atm2ice_map)
          end if
          call addmrgTo(compice, 'Sa_v', mrg_from=compatm, mrg_fld='Sa_v', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_shum')
       call addfldTo(compice, 'Sa_shum')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_shum', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_shum', rc=rc)) then
          call addMapFrom(compatm, 'Sa_shum', compice, mapbilnr, 'one', atm2ice_map)
          call addmrgTo(compice, 'Sa_shum', mrg_from=compatm, mrg_fld='Sa_shum', mrg_type='copy')
       end if
    end if
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Sa_shum_wiso')
          call addfldTo(compice, 'Sa_shum_wiso')
       else
          if ( fldchk(is_local%wrap%FBexp(compice)         , 'Sa_shum_wiso', rc=rc) .and. &
               fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_shum_wiso', rc=rc)) then
             call addMapFrom(compatm, 'Sa_shum_wiso', compice, mapbilnr, 'one', atm2ice_map)
             call addmrgTo(compice, 'Sa_shum_wiso', mrg_from=compatm, mrg_fld='Sa_shum_wiso', mrg_type='copy')
          end if
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_t')
       call addfldTo(compice, 'So_t')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_t', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_t', rc=rc)) then
          call addmapFrom(compocn, 'So_t', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_t', mrg_from=compocn, mrg_fld='So_t', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_s')
       call addfldTo(compice, 'So_s')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_s', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_s', rc=rc)) then
          call addmapFrom(compocn, 'So_s', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_s', mrg_from=compocn, mrg_fld='So_s', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_u')
       call addfldTo(compice, 'So_u')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_u', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_u', rc=rc)) then
          call addmapFrom(compocn, 'So_u', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_u', mrg_from=compocn, mrg_fld='So_u', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_v')
       call addfldTo(compice, 'So_v')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_v', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_v', rc=rc)) then
          call addmapFrom(compocn, 'So_v', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_v', mrg_from=compocn, mrg_fld='So_v', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_dhdx')
       call addfldTo(compice, 'So_dhdx')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_dhdx', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_dhdx', rc=rc)) then
          call addmapFrom(compocn, 'So_dhdx', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_dhdx', mrg_from=compocn, mrg_fld='So_dhdx', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_dhdy')
       call addfldTo(compice, 'So_dhdy')
    else
       if ( fldchk(is_local%wrap%FBexp(compice)        , 'So_dhdy', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compocn,compocn), 'So_dhdy', rc=rc)) then
          call addmapFrom(compocn, 'So_dhdy', compice, mapfcopy , 'unset', 'unset')
          call addmrgTo(compice, 'So_dhdy', mrg_from=compocn, mrg_fld='So_dhdy', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'Fioo_q')
       call addfldTo(compice, 'Fioo_q')
    else
       if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'Fioo_q', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compice)         , 'Fioo_q', rc=rc)) then
          call addmapFrom(compocn, 'Fioo_q', compice,  mapfcopy, 'unset', 'unset')
          call addmrgTo(compice, 'Fioo_q', mrg_from=compocn, mrg_fld='Fioo_q', mrg_type='copy')
       end if
    end if
    !-----------------------------
    ...
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addfldFrom(compocn, 'So_roce_wiso')
          call addfldTo(compice, 'So_roce_wiso')
       else
          if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'So_roce_wiso', rc=rc) .and. &
             ...
             call addmapFrom(compocn, 'So_roce_wiso', compice,  mapfcopy, 'unset', 'unset')
             call addmrgTo(compice, 'So_roce_wiso', mrg_from=compocn, mrg_fld='So_roce_wiso', mrg_type='copy')
          end if
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(comprof, 'Firr_rofi') ! water flux into sea ice due to runoff (frozen)
       do ns = 1, is_local%wrap%num_icesheets
          call addfldFrom(compglc(ns), 'Figg_rofi') ! glc frozen runoff_iceberg flux to ice
       end do
       call addfldTo(compice, 'Fixx_rofi') ! total frozen water flux into sea ice
    else
       if ( fldchk(is_local%wrap%FBExp(compice), 'Fixx_rofi', rc=rc)) then
          if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofi', rc=rc)) then
             call addmapFrom(comprof, 'Forr_rofi', compice, mapconsf, 'none', rof2ocn_ice_rmap)
             call addmrgTo(compice, 'Fixx_rofi', mrg_from=comprof, mrg_fld='Firr_rofi', mrg_type='sum')
          end if
          do ns = 1, is_local%wrap%num_icesheets
             if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Figg_rofi', rc=rc)) then
                call addmapFrom(compglc(ns), 'Figg_rofi', compice, mapconsf, 'one' , glc2ice_rmap)
                call addmrgTo(compice, 'Fixx_rofi', mrg_from=compglc(ns), mrg_fld='Figg_rofi', mrg_type='sum')
             end if
          end do
       end if
    end if
    if (flds_wiso) then
       if (phase == 'advertise') then
          call addfldFrom(comprof, 'Firr_rofi_wiso') ! water flux into sea ice due to runoff (frozen)
          do ns = 1, is_local%wrap%num_icesheets
             call addfldFrom(compglc(ns), 'Figg_rofi_wiso') ! glc frozen runoff_iceberg flux to ice
          end do
          call addfldTo(compice, 'Fixx_rofi_wiso') ! total frozen water flux into sea ice
       else
          if ( fldchk(is_local%wrap%FBExp(compice), 'Fixx_rofi_wiso', rc=rc)) then
             if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofi_wiso', rc=rc)) then
                call addmapFrom(comprof, 'Forr_rofi_wiso', compice, mapconsf, 'none', rof2ocn_ice_rmap)
                call addmrgTo(compice, 'Fixx_rofi_wiso', &
             ...
             do ns = 1, is_local%wrap%num_icesheets
                if (fldchk(is_local%wrap%FBImp(compglc(ns), compglc(ns)), 'Figg_rofi_wiso', rc=rc)) then
                   call addmapFrom(compglc(ns), 'Figg_rofi_wiso', compice, mapconsf, 'one' , glc2ice_rmap)
                   call addmrgTo(compice, 'Fixx_rofi_wiso', &
    ...
    if (wavice_coupling) then
       if (phase == 'advertise') then
          ...
          if ( fldchk(is_local%wrap%FBExp(compice)        , 'Sw_elevation_spectrum', rc=rc) .and. &
             ...
             call addmapFrom(compwav, 'Sw_elevation_spectrum', compice, mapbilnr, 'one', 'unset')
             call addmrgTo(compice, 'Sw_elevation_spectrum', &
    ...
    if (phase == 'advertise') then
       call addfldFrom(compice, 'Si_ifrac')
       call addfldTo(compwav, 'Si_ifrac')
    else
       if ( fldchk(is_local%wrap%FBexp(compwav)         , 'Si_ifrac', rc=rc) .and. &
          ...
          call addMapFrom(compice, 'Si_ifrac', compwav, mapbilnr, 'one', ice2wav_smap)
          call addmrgTo(compwav, 'Si_ifrac', mrg_from=compice, mrg_fld='Si_ifrac', mrg_type='copy')
       end if
    end if
    !----------------------------------------------------------
    ...
    if (wavice_coupling) then
       if (phase == 'advertise') then
          call addfldFrom(compice, 'Si_thick')
          call addfldTo(compwav, 'Si_thick')
       else
          if (fldchk(is_local%wrap%FBexp(compwav)         , 'Si_thick', rc=rc) .and. &
              fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_thick', rc=rc)) then
             call addMapFrom(compice, 'Si_thick', compwav, mapbilnr, 'one', ice2wav_smap)
             call addmrgTo(compwav, 'Si_thick', mrg_from=compice, mrg_fld='Si_thick', mrg_type='copy')
          end if
       end if
    end if
    !----------------------------------------------------------
    ...
    if (wavice_coupling) then
       if (phase == 'advertise') then
          call addfldFrom(compice, 'Si_floediam')
          call addfldTo(compwav, 'Si_floediam')
       else
          if (fldchk(is_local%wrap%FBexp(compwav)         , 'Si_floediam', rc=rc) .and. &
             ...
             call addMapFrom(compice, 'Si_floediam', compwav, mapbilnr, 'one', ice2wav_smap)
             call addmrgTo(compwav, 'Si_floediam', mrg_from=compice, mrg_fld='Si_floediam', mrg_type='copy')
          end if
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_t')
       call addfldTo(compwav, 'So_t')
    else
       if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'So_t', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compwav)         , 'So_t', rc=rc)) then
          ! By default will be using a custom map - but if one is not available, use a generated bilinear instead
          call addmapFrom(compocn, 'So_t', compwav, mapbilnr, 'one', ocn2wav_smap)
          call addmrgTo(compwav, 'So_t', mrg_from=compocn, mrg_fld='So_t', mrg_type='copy')
       end if
    end if
    ! ---------------------------------------------------------------------
    ...
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_u')
       call addfldTo(compwav, 'So_u')
    else
       if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'So_u', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compwav)         , 'So_u', rc=rc)) then
          ! By default will be using a custom map - but if one is not available, use a generated bilinear instead
          call addmapFrom(compocn, 'So_u', compwav, mapbilnr, 'one', ocn2wav_smap)
          call addmrgTo(compwav, 'So_u', mrg_from=compocn, mrg_fld='So_u', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_v')
       call addfldTo(compwav, 'So_v')
    else
       if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'So_v', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(compwav)         , 'So_v', rc=rc)) then
          ! By default will be using a custom map - but if one is not available, use a generated bilinear instead
          call addmapFrom(compocn, 'So_v', compwav, mapbilnr, 'one', ocn2wav_smap)
          call addmrgTo(compwav, 'So_v', mrg_from=compocn, mrg_fld='So_v', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addfldFrom(compocn, 'So_bldepth')
       call addfldTo(compwav, 'So_bldepth')
    else
       if ( fldchk(is_local%wrap%FBImp(compocn, compocn), 'So_bldepth', rc=rc) .and. &
          ...
          ! By default will be using a custom map - but if one is not available, use a generated bilinear instead
          call addmapFrom(compocn, 'So_bldepth', compwav, mapbilnr, 'one', ocn2wav_smap)
          call addmrgTo(compwav, 'So_bldepth', mrg_from=compocn, mrg_fld='So_bldepth', mrg_type='copy')
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_u')
       call addfldTo(compwav, 'Sa_u')
    else
       if ( fldchk(is_local%wrap%FBexp(compwav)         , 'Sa_u', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_u', rc=rc)) then
          call addMapFrom(compatm, 'Sa_u', compwav, mapbilnr, 'one', atm2wav_map)
          call addmrgTo(compwav, 'Sa_u', mrg_from=compatm, mrg_fld='Sa_u', mrg_type='copy')
       end if
    end if
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_v')
       call addfldTo(compwav, 'Sa_v')
    else
       if ( fldchk(is_local%wrap%FBexp(compwav)         , 'Sa_v', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_v', rc=rc)) then
          call addMapFrom(compatm, 'Sa_v', compwav, mapbilnr, 'one', atm2wav_map)
          call addmrgTo(compwav, 'Sa_v', mrg_from=compatm, mrg_fld='Sa_v', mrg_type='copy')
       end if
    end if
    ...
    if (phase == 'advertise') then
       call addFldFrom(compatm, 'Sa_tbot')
       call addfldTo(compwav, 'Sa_tbot')
    else
       if ( fldchk(is_local%wrap%FBexp(compwav)         , 'Sa_tbot', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compatm,compatm ), 'Sa_tbot', rc=rc)) then
          call addMapFrom(compatm, 'Sa_tbot', compwav, mapbilnr, 'one', atm2wav_map)
          call addmrgTo(compwav, 'Sa_tbot', mrg_from=compatm, mrg_fld='Sa_tbot', mrg_type='copy')
       end if
    end if
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Flrl_rofsur')
       call addfldTo(comprof, 'Flrl_rofsur')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Flrl_rofsur', rc=rc) .and. &
          ...
          call addmapFrom(complnd, 'Flrl_rofsur', comprof, mapconsf, 'lfrac', lnd2rof_map)
          call addmrgTo(comprof, 'Flrl_rofsur', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Flrl_rofi')
       call addfldTo(comprof, 'Flrl_rofi')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Flrl_rofi', rc=rc) .and. &
            fldchk(is_local%wrap%FBExp(comprof)         , 'Flrl_rofi', rc=rc)) then
          call addmapFrom(complnd, 'Flrl_rofi', comprof, mapconsf, 'lfrac', lnd2rof_map)
          call addmrgTo(comprof, 'Flrl_rofi', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Flrl_rofgwl')
       call addfldTo(comprof, 'Flrl_rofgwl')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Flrl_rofgwl', rc=rc) .and. &
          ...
          call addmapFrom(complnd, 'Flrl_rofgwl', comprof, mapconsf, 'lfrac', lnd2rof_map)
          call addmrgTo(comprof, 'Flrl_rofgwl', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Flrl_rofsub')
       call addfldTo(comprof, 'Flrl_rofsub')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Flrl_rofsub', rc=rc) .and. &
          ...
          call addmapFrom(complnd, 'Flrl_rofsub', comprof, mapconsf, 'lfrac', lnd2rof_map)
          call addmrgTo(comprof, 'Flrl_rofsub', &
    ...
    ! ---------------------------------------------------------------------
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Flrl_irrig')
       call addfldTo(comprof, 'Flrl_irrig')
    else
       if ( fldchk(is_local%wrap%FBImp(complnd, complnd), 'Flrl_irrig', rc=rc) .and. &
          ...
          call addmapFrom(complnd, 'Flrl_irrig', comprof, mapconsf, 'lfrac', lnd2rof_map)
          call addmrgTo(comprof, 'Flrl_irrig', &
    ...
    if (phase == 'advertise') then
       call addFldFrom(complnd, 'Sl_tsrf_elev')   ! surface temperature of glacier (1->glc_nec+1)
       call addFldFrom(complnd, 'Sl_topo_elev')   ! surface heights of glacier     (1->glc_nec+1)
       call addFldFrom(complnd, 'Flgl_qice_elev') ! glacier ice flux               (1->glc_nec+1)
       do ns = 1,is_local%wrap%num_icesheets
          call addfldTo(compglc(ns), 'Sl_tsrf')
          call addfldTo(compglc(ns), 'Flgl_qice')
       end do
    else
       ...
       do ns = 1,is_local%wrap%num_icesheets
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd) , 'Flgl_qice_elev', rc=rc)) then
             call addmapFrom(complnd, 'Flgl_qice_elev', compglc(ns), mapbilnr, 'lfrac', 'unset')
          end if
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd) , 'Sl_tsrf_elev'  , rc=rc)) then
             call addmapFrom(complnd, 'Sl_tsrf_elev', compglc(ns), mapbilnr, 'lfrac', 'unset')
          end if
          if ( fldchk(is_local%wrap%FBImp(complnd,complnd) , 'Sl_topo_elev'  , rc=rc)) then
             ...
             call addmapFrom(complnd, 'Sl_topo_elev', compglc(ns), mapbilnr, 'lfrac', 'unset')
          end if
       end do
    end if
    ...
    if (is_local%wrap%ocn2glc_coupling) then
       if (phase == 'advertise') then
          call addfldFrom(compocn, 'So_t_depth')
          call addfldFrom(compocn, 'So_s_depth')
          do ns = 1,is_local%wrap%num_icesheets
             call addfldTo(compglc(ns), 'So_t_depth')
             call addfldTo(compglc(ns), 'So_s_depth')
          end do
       else
          ! custom mapping, accumulation and merging will be done in prep_glc_mod.F90
          ...
          do ns = 1,is_local%wrap%num_icesheets
             if ( fldchk(is_local%wrap%FBImp(compocn,compocn) , 'So_t_depth', rc=rc)) then
                call addmapFrom(compocn, 'So_t_depth', compglc(ns), mapbilnr, 'none', 'unset')
             end if
             if ( fldchk(is_local%wrap%FBImp(compocn,compocn) , 'So_s_depth', rc=rc)) then
                call addmapFrom(compocn, 'So_s_depth', compglc(ns), mapbilnr, 'none', 'unset')
             end if
          end do
       end if
    end if
    ...
    if (flds_co2a) then
       ...
       ! ---------------------------------------------------------------------
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Sa_co2prog')
          call addfldTo(complnd, 'Sa_co2prog')
          call addFldTo(compocn, 'Sa_co2prog')
       else
          call addMapFrom(compatm, 'Sa_co2prog', complnd, mapbilnr, 'one', atm2lnd_map)
          call addMapFrom(compatm, 'Sa_co2prog', compocn, mapbilnr, 'one', atm2ocn_map)

          call addmrgTo(complnd, 'Sa_co2prog', &
               mrg_from=compatm, mrg_fld='Sa_co2prog', mrg_type='copy')
          call addmrgTo(compocn, 'Sa_co2prog', &
               mrg_from=compatm, mrg_fld='Sa_co2prog', mrg_type='copy')
       end if
       ...
       ! ---------------------------------------------------------------------
       if (phase == 'advertise') then
          call addFldFrom(compatm, 'Sa_co2diag')
          call addfldTo(complnd, 'Sa_co2diag')
          call addFldTo(compocn, 'Sa_co2diag')
       else
          call addMapFrom(compatm, 'Sa_co2diag', complnd, mapbilnr, 'one', atm2lnd_map)
          call addMapFrom(compatm, 'Sa_co2diag', compocn, mapbilnr, 'one', atm2ocn_map)

          call addmrgTo(complnd, 'Sa_co2diag', &
               mrg_from=compatm, mrg_fld='Sa_co2diag', mrg_type='copy')
          call addmrgTo(compocn, 'Sa_co2diag', &
               mrg_from=compatm, mrg_fld='Sa_co2diag', mrg_type='copy')
       end if
       ...
       ! ---------------------------------------------------------------------
       if (phase == 'advertise') then
          call addFldFrom(complnd, 'Fall_fco2_lnd')
          call addfldTo(compatm, 'Fall_fco2_lnd')
       else
          call addmapFrom(complnd, 'Fall_fco2_lnd', compatm, mapconsf, 'one', lnd2atm_map)
          call addmrgTo(compatm, 'Fall_fco2_lnd', &
       ...
       ! ---------------------------------------------------------------------
       if (phase == 'advertise') then
          call addfldFrom(compocn, 'Faoo_fco2_ocn')
          call addfldTo(compatm, 'Faoo_fco2_ocn')
       else
          call addmapFrom(compocn, 'Faoo_fco2_ocn', compatm, mapconsd, 'one', ocn2atm_map)
