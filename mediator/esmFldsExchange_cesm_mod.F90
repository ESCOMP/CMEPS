...
!================================================================================
  ...
  subroutine esmFldsExchange_cesm(gcomp, phase, rc)
    ...
    ! Determine wave model name
    call NUOPC_CompAttributeGet(gcomp, name='WAV_model', value=wav_model, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    if (phase == 'advertise') then
       ...
       ! mapping to atm
       if (trim(mesh_atm) == trim(mesh_ocn)) then
          ice2atm_map = 'idmap'
          ocn2atm_map = 'idmap'
       else
       ...
       if (trim(mesh_atm) == trim(mesh_lnd)) then
          lnd2atm_map = 'idmap'
       else
    ...
    ! ---------------------------------------------------------------------
    if (is_local%wrap%comp_present(complnd) .and. is_local%wrap%comp_present(comprof)) then
       if (phase == 'advertise') then
    ...
    if (is_local%wrap%comp_present(complnd) .and. is_local%wrap%num_icesheets > 0) then
       if (phase == 'advertise') then
          do ns = 1, is_local%wrap%num_icesheets
          ...
          if ( fldchk(is_local%wrap%FBExp(compocn), 'Foxx_rofl' , rc=rc)) then
             ...
             if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Forr_rofl' , rc=rc)) then
                ...
                if (fldchk(is_local%wrap%FBImp(comprof, comprof), 'Flrr_flood', rc=rc)) then
                   ...
                   call addmrg(fldListTo(compocn)%flds, 'Foxx_rofl', mrg_from=comprof, &
                        mrg_fld='Forr_rofl:Flrr_flood', mrg_type='sum')
                else
                   call addmrg(fldListTo(compocn)%flds, 'Foxx_rofl', mrg_from=comprof, &
                        mrg_fld='Forr_rofl', mrg_type='sum')
                end if
             end if
    ...
    !-----------------------------
    ! TODO: note that current dwav does not send Sw_hstokes
    if (is_local%wrap%comp_present(compocn) .and. is_local%wrap%comp_present(compwav) .and. trim(wav_model) /= 'dwav') then
       if (phase == 'advertise') then
    ...
    !-----------------------------
    ! TODO: note that current dwav does not send Sw_pstokes_x
    if (is_local%wrap%comp_present(compocn) .and. is_local%wrap%comp_present(compwav) .and. trim(wav_model) /= 'dwav') then
       if (phase == 'advertise') then
    ...
    ! to ocn: Stokes drift depth from wave
    !-----------------------------
    ! TODO: note that current dwav does not send Sw_pstokes_y
    if (is_local%wrap%comp_present(compocn) .and. is_local%wrap%comp_present(compwav) .and. trim(wav_model) /= 'dwav') then
       if (phase == 'advertise') then
    ...
    if (is_local%wrap%num_icesheets > 0 .and. is_local%wrap%comp_present(complnd)) then
       if (phase == 'advertise') then
    ...
    if (flds_co2a) then
       ...
       ! to atm: surface flux of CO2 from land
       ! ---------------------------------------------------------------------
       ! TODO: if the land atm atm are on different grids - the following will not work with packed data structures
       ! since the
       if (phase == 'advertise') then
          call addfld(fldListFr(complnd)%flds, 'Fall_fco2_lnd')
          call addfld(fldListTo(compatm)%flds, 'Fall_fco2_lnd')
